...
class HobbiesRepository(tokenManager: TokenManager) {
    ...
    suspend fun getAllHobbies(userProfileId: Long):ApiResult<List<String>>{
        return safeApiCall { retrofitAPI.getAllHobbies(userProfileId).execute() }
    }

    suspend fun addHobbies(hobbies: List<String>, userProfileId:Long):ApiResult<List<String>> {
        return safeApiCall { retrofitAPI.addHobbies(hobbies,userProfileId).execute() }
    }
}